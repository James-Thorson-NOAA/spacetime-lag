...
###############
# NOTE
# Using existing copy of data_sf for each species to freeze results from original submission
##############
...
#
Date = Sys.Date()
#Date = "2026-06-23_cutoff=40_prof=none_intercept=constant_resids"
...
if( !("settings.RDS" %in% list.files(date_dir)) ){
  ...
  cutoff1 = 40
  cutoff2 = 40
  intercept = c("constant", "annual")[1]
  profile = NULL   # c("gamma_j")  # , # "beta_k","log_kappaS","rho_t","rho_st")
  #profile = "beta_k"   # c("gamma_j")  # , # "beta_k","log_kappaS","rho_t","rho_st")
  ...
  run_sdreport = TRUE # must be TRUE for summaries to work
  L1version = c("innovations", "stationary")[2]   # stationary:  Lag matrix has columns that sum to 0

  # settings
  settings = mget(c(
    "rescale", "cutoff1", "cutoff2", "profile","covariate", "log_kappaS_start",
    "config_set", "version", "hesscompress", "run_sdreport", "L1version", "intercept" ))
...
#for( ci in 5:6 ){
  ...
  if( !("opt.RDS" %in% list.files(run_dir)) ){
    ...
    #data = na.omit(data[,c('lat_start','lon_start','year','bottom_temp_c','catch_weight','effort')])

    # Restrict years (to match original submission)
    #data = data[ which(data$year %in% year_set), ]
    ...
    data_sf = readRDS( file = file.path(data_dir, species, "data_sf.RDS") )
    ...
    #
    if( intercept == "constant" ){
      formula = ~ 1
    }else if( intercept == "annual" ){
      formula = ~ 0 + factor(data_sf$year)
    }
    ...
    # Make inputs
    data = list(
      options_z = c( switch(L1version, "innovations"=0, "stationary"=1), do_quadratic ),
      n_t = n_t,
    ...
    if( class(opt) == "try-error" ){
      ...
      # Skip this here, and run below
      #nsim = 1000
      #par_iz = outer( obj$env$last.par.best, rep(1,nsim) )
      #b_iz = apply(
      #  par_iz,
      #  MARGIN = 2,
      #  FUN = function(par_i) obj$simulate(par_i)$b_i
      #)
      #saveRDS(b_iz, file = file.path(run_dir,"b_iz.RDS") )
...
p_sczq = array( NA, dim=c(length(species_set),nrow(config_set),length(Temp_z),3), 
...
png( file = file.path(date_dir,"summary.png"), width = 1.1 * length(species_set), height = 1.5 * 3, res = 200, units = "in" )
  ...
  for( si in seq_along(species_set) ){
    # AIC weights
    plot( x = seq_len(nrow(config_set)), y = results_scz[si,,"AIC_weight"], xlab = "", ylab = "", xlim = xlim,
    ...
    if(si==1) axis(2)
    # rho_t
    plot( x = seq_len(nrow(config_set)), y = results_scz[si,,"rhoT"], xlab = "", ylab = "", xlim = xlim, xaxt = "n",
...
png( file = file.path(date_dir,"marginal_response.png"), width = 2 * 2, height = 2 * 3, res = 200, units = "in" )
  ...
  for( si in seq_along(species_set) ){
    ...
    y_czq = exp(y_czq)
    if( length(ci) > 0 ){
      matplot( x = Temp_z, y = t(y_czq[c(1,ci),,'mid']), lwd = 2, col = c("black","blue"),
  ...
  mtext( side = 1:2, text = c( ifelse(covariate == "temp", "Seafloor temperature (C)", "Seafloor temperature anomaly (C)"),
                               "Density relative to average"), outer = TRUE )
...
if( covariate == "anomaly" ){
  ...
  png( file = file.path(date_dir,"Temp_gt.png"), width = 10, height = 10, res = 200, units = "in")
    ...
    for( t in seq_len(ncol(Temp_gt)) ){
      plot( plot_grid[,t], border = NA, key.pos=NULL, reset=FALSE,
            breaks = seq(-1*maxval, maxval, length = 11), pal = pal(11) )  # , zlim = range(cbind(Temp_gt))
      box()
...
#which_year = 2014
T_scgt = gamma_scgt = array(NA, dim=c(length(species_set),nrow(config_set),length(grid),length(year_set)),
...
n_sim = 100
ps_ic = vector("list", length = length(species_set))
for( si in seq_along(species_set) ){
  species = species_set[si]
  data_sf = readRDS( file = file.path(data_dir, species, "data_sf.RDS") )
  ps_ic[[si]] = array( NA, dim = c(nrow(data_sf),nrow(config_set)) )
  for( ci in seq_len(nrow(config_set)) ){
    #
    do_space = config_set[ci,'space']
    do_time = config_set[ci,'time']
    ...
    if( "opt.RDS" %in% list.files(run_dir) ){
      obj = readRDS( file.path(run_dir,"obj.RDS") )
      b_iz = sapply( seq_len(n_sim), FUN = \(x) obj$env$simulate()$b_i )
      ps_ic[[si]][,ci] = runif(
        n = nrow(b_iz),
        min = rowMeans(b_iz < data_sf$catch_weight),
        max = rowMeans(b_iz <= data_sf$catch_weight)
      )
    }
  }
}
png( file = file.path(date_dir,"quantile_residuals.png"), width = 2*nrow(config_set), height = 2*length(species_set), res = 200, units = "in")
  ...
  for( si in seq_along(species_set) ){
  for( ci in seq_len(nrow(config_set)) ){
    qqplot(ps_ic[[si]][,ci], y=seq(0,1,length = nrow(data_sf)), xlab = "", ylab = "", xaxt = "n", yaxt = "n", type = "l", lwd = 2 )
    abline( a = 0, b = 1, lty = "dotted", lwd = 0.8 )
    if(ci==1) mtext( side=2, text = species_names[si] )
    if(ci==nrow(config_set)) axis(4)
    if(si==1) mtext( side=3, text = model_names[ci] )
    if(si==length(species_set)) axis(1)
    box()
  }}
